...
Crimes <- Crimes[-c(1:2,4:5,7:8,11:13,15:22)] 

# Splitting Date column into Date and Time and AM/PM
Crimes$Date_2 <- sapply(strsplit(as.character(Crimes$Date), " "), "[", 1)
Crimes$Time <- sapply(strsplit(as.character(Crimes$Date), " "), "[", 2)
Crimes$AM.PM <- sapply(strsplit(as.character(Crimes$Date), " "), "[", 3)
...
#Keeping only observations with community area 60 and 34
Crimes_Games <- Crimes_Games[which(Crimes_Games$Community.Area == 34 | Crimes_Games$Community.Area == 60),]

#Aggregate
table1 <- data.frame(table(Crimes_Games$Date))
names(table1)[1] <- 'Date'


final <- merge(Crimes_Games, table1, by = 'Date')



# Fitted regression tests

p <- lm(formula = Freq ~ Primary.Type + Day + Gameday, data = final)
summary(p)


q <- lm(formula = Freq ~ Gameday + Day, data=final)
lm(q)
...
merge1$Time <- format(as.POSIXct(as.character(merge1$Time)), "%H:%M:%S")
...
for (i in 1:nrow(Crimes_Games)) {
  
  if (Crimes_Games$AM.PM == 'AM' & Crimes_Games$Time ) {
    
    Crimes_Games$Day.Night == 'D' }
...
lm(formula = Freq ~ Date + Primary.Type + Arrest + Domestic + Community.Area + Day + Gameday, data=merge1)

summary


Date <- seq.Date(from=as.Date("2001-01-01"), to=as.Date("2021-09-04"), by="day")
